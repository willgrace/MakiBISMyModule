...
class User_model extends CI_Model {
	...
	private static $search_terms = array('s_key_one');
...
/*added by graycel-----------SEARCH ALL*/
	function get_search($limit,$start,$search){
    	/*$this->db->limit($limit,$start);
		
    	$this->db->like('habitat', $search);
    	$this->db->from('habitat');
    ...
    function get_habitat($q){
	    $this->db->select('habitat');
	    $this->db->like('habitat', $q);
	    $query = $this->db->get('habitat');
	    if($query->num_rows > 0){
	      foreach ($query->result_array() as $row){
	        $row_set[] = htmlentities(stripslashes($row['habitat'])); //build an array
	      }
	...
	function _filter(){
		...
		if($this->s_key_one !== FALSE){
			...
			//$this->db->where(" applications.key_one = " . $this->s_key_one . " ");
 			$this->db->like('habitat', $this->s_key_one);
    		$this->db->from('habitat');
		}
