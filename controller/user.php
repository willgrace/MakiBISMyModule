...
class User extends CI_Controller {
	...
	/*--------------SEARCH ALL----------added by graycel----------------*/

	function search_All(){
		//if($this->session->userdata('logged_in_user')||$this->session->userdata('logged_in_admin')){
			//$flag = 'species';
			$this->form_validation->set_rules('searchfield','Search anything','alpha|required|xss_clean');
			...
			if($this->form_validation->run() == FALSE){
				//$data['heading'] = "Search Results";
				//$this->load->view('v_ok',$data);
				$data['heading'] = "Search";
				$this->load->view('v_taxon_tree');
			}
			else{
			
				redirect('user/search_results_tree/'.$search);
			}
		//}
		//else{
			//redirect(base_url());
	...
	function search_results_tree($search){
    	//if($this->session->userdata('logged_in_user')||$this->session->userdata('logged_in_admin')){
		$this->load->library('pagination');
				/*$config = array();
				$config["base_url"] = 'http://localhost/NewMakiBIS/index.php/user/search_results_tree/'.$search;
				$config["total_rows"] = $this->user_model->count_results($search);
				...
				$data['results'] = $this->user_model->get_search($config["per_page"],$page,$search);
				$data['links'] = $this->pagination->create_links();
				$data['search']=$search;
				
				$this->load->view('v_result_tree',$data);*/ 
	...
	function search_habitat(){
	   $this->load->model('user_model');
	   if (isset($_GET['term'])){
	     $q = strtolower($_GET['term']);
	     $this->user_model->get_habitat($q);
	   }
	}
...
//net
	...
	function search(){
		...
		/*if( ! isset($_POST['submit']) ){
			...
			$this->session->unset_userdata('s_key_one');
			//$this->session->unset_userdata('s_key_two');
			//$this->session->unset_userdata('s_key_three');
			...
			// this is how it works if you store your validation rules in a file
			// if you do, good, if not, make sure that you have a file that contains your
			// validation rules in a format that can be found in CI's user guide

			//$this->config->load('form_validation', TRUE);
			$this->form_validation->set_rules('searchfield','Search anything','alpha|required|xss_clean');

			//$search = $this->input->post('s_key_one');		
			...
			if ($this->form_validation->run() == FALSE){
				...
				$this->terms['s_key_one'] = $this->input->post('searchfield');
				//$this->terms['s_key_two'] = $this->input->post('key_two');
				//$this->terms['s_key_three'] = $this->input->post('key_three');
	...
	function view(){
		...
		$data['results'] = $this->user_model->get_all($this->results_per_page, $offset);
 
		$data['total_rows'] = $total;
 
		$config['base_url'] = 'http://localhost/NewMakiBIS/index.php/user/view/';
